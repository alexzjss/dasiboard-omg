...
function runInSandbox(code, testFn, fnName, timeoutMs = 3000) {
    const sandbox = { results: [], console: { log: () => { } } };
    const script = `
    (function() {
      try {
        ${code}
  ...
  `;
    try {
        vm_1.default.runInNewContext(script, sandbox, { timeout: timeoutMs });
    }
    catch (e) {
...
const submitSchema = zod_1.z.object({
    lang: zod_1.z.nativeEnum(enums_1.ChallengeLanguage),
...
router.get('/progress', auth_1.requireAuth, async (req, res, next) => {
    try {
        const subs = await prisma_1.prisma.challengeSubmission.findMany({
        ...
        const progress = {};
        for (const s of subs)
            progress[s.challengeId] = s.passed;
        res.json({ progress });
    }
    catch (err) {
        next(err);
...
router.post('/:id/submit', auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const submission = await prisma_1.prisma.challengeSubmission.create({
            data: {
                userId: req.user.sub,
